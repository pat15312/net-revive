...
try{
 ...
 await secondSwitch.getByRole('button',{name:'Edit label & availability'}).click();
 ...
 assert.equal(await page.locator('#target-inventory [data-target="3"]').evaluate(el=>el===document.activeElement),true);
 const editedTargets=(await (await page.request.get('http://127.0.0.1:8019/api/admin/config')).json()).targets;
 assert.equal(editedTargets.find(t=>t.id===3).label,'Firewall');
