...
function renderInventory() {
  ...
  inventory.innerHTML = targets.length
    ? `<div class="section-head"><h3>Discovered PoE ports</h3><span class="muted">${targets.length} ports · ${switches.size} ${switches.size === 1 ? "switch" : "switches"}</span></div>${[...switches].map(([key, device]) =>
      `<details class="inventory-switch" data-switch="${esc(key)}" ${expandedInventorySwitches.has(key) ? "open" : ""}><summary>${esc(device.name)} <span class="muted switch-count">${device.ports.length} ${device.ports.length === 1 ? "port" : "ports"}</span></summary><div class="switch-ports" role="region" aria-label="${esc(device.name)} ports" tabindex="0"><div class="port-grid">${device.ports.sort((a, b) => a.port_number - b.port_number).map(t =>
        `<button type="button" class="port-tile ${!t.enabled ? "port-disabled" : t.available ? "port-available" : "port-unavailable"}" data-target="${t.id}" title="${esc(`${device.name} · Port ${t.port_number}${t.label || t.port_name ? ' · ' + (t.label || t.port_name) : ''} · ${!t.enabled ? 'Disabled' : t.available ? 'Available' : 'Unavailable'}`)}" aria-label="${esc(`${device.name}, Port ${t.port_number}${t.label || t.port_name ? ', ' + (t.label || t.port_name) : ''}, ${!t.enabled ? 'Disabled' : t.available ? 'Available' : 'Unavailable'}. Edit port`)}"><span class="port-number">${t.port_number}</span><span class="port-socket" aria-hidden="true"></span><span class="port-label">${esc(t.label || t.port_name || "")}</span></button>`
      ).join("")}</div></div><div class="port-legend"><span>Click a port to edit</span><span>● Available</span><span>◇ Unavailable</span><span>— Disabled</span></div></details>`
    ).join("")}`
