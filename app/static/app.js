...
function unifiView() {
  const s = config.settings;
  $("#admin-content").innerHTML =
    `<section class="panel"><h2>Connect to UniFi</h2><p>Use the address of your local UniFi controller. NetRevive communicates with it directly.</p><form id="unifi-form">${field("Controller URL", "controller_url", s.controller_url, "url", 'required placeholder="https://controller.lan"')}<label for="api_prefix">Controller type</label><select name="api_prefix" id="api_prefix"><option value="/proxy/network/integration/v1" ${s.api_prefix.includes("proxy") ? "selected" : ""}>UniFi OS console</option><option value="/integration/v1" ${!s.api_prefix.includes("proxy") ? "selected" : ""}>Self-hosted Network application</option></select>${s.api_key_from_environment ? '<p class="notice">The API key is managed through the container environment.</p>' : field(s.api_key_configured ? "Replace API key (leave blank to keep it)" : "API key", "api_key", "", "password", 'autocomplete="off" maxlength="4096"')}<p class="help">Create a local API key in UniFi Network → Settings → Control Plane → Integrations. Your key is never returned to the browser.</p>${check("Verify the controller’s TLS certificate (recommended)", "verify_tls", s.verify_tls)}<p class="help">For a private CA, install its certificate in the container. Disabling verification makes the connection vulnerable to impersonation.</p><div class="actions"><button class="primary" type="submit">Save connection</button><button class="secondary" type="button" id="test-unifi">Test UniFi connection</button></div><div id="connection-result" role="status"></div></form></section><section class="panel"><h2>Site & PoE equipment</h2><p>Select a site, then discover its switches and restartable ports.</p><form id="site-form"><label for="site">UniFi site</label><select id="site" name="site_id" required><option value="">Choose a site</option>${sites.map((site) => `<option value="${esc(site.id)}" ${site.id === s.site_id ? "selected" : ""}>${esc(site.name)}</option>`).join("")}${s.site_id && !sites.some((x) => x.id === s.site_id) ? `<option selected value="${esc(s.site_id)}">Configured site — test connection to refresh names</option>` : ""}</select><div class="actions"><button class="primary" type="submit">Save site & discover ports</button></div></form><div id="target-inventory"></div></section>`;
